(function () {
    ...
    window.app.directive('deleteAmortisasirequest', deleteAmortisasirequest);
    function deleteAmortisasirequest() {
        return {
            scope: {
                data: '='
            },
            templateUrl: MyApplication.rootPath + '/amortisasirequest/template/delete.tmpl.cshtml',
    ...
    function controller($scope, $modal, AmortisasiRequestService) {
        ...
        //        }
        //        vm.totalBeban = vm.amortisasirequest.pO_NominalBarang + (10 / 100 * vm.amortisasirequest.pO_NominalBarang)
        ...
        vm.delete = function () {
            vm.saving = true;
            AmortisasiRequestService.deleteData(vm.amortisasirequest)
				.then(function () {
				    //Close the modal
				    $scope.$parent.$close();
				})
