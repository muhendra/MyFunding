(function () {
    window.app.factory('branchService', branchService);

    branchService.$inject = ['$http', 'DataService', 'alerts'];
    function branchService($http, DataService, alerts) {
        var controllerPath = 'Branch/'
        var list = [];               
        ...
        function getData(id) {
            for (var i = 0; i < list.length; i++) {
                if (list[i].branchId == id) return list[i];
            }
