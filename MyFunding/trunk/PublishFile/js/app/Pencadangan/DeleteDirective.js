(function () {
    ...
    window.app.directive('deletePencadangan', deletePencadangan);
    function deletePencadangan() {
        return {
            scope: {
                data: '='
            },
            templateUrl: MyApplication.rootPath + '/pencadangan/template/delete.tmpl.cshtml',
    ...
    function controller($scope, $modal, PencadanganService) {
        ...
        vm.setKartuAmortisasi = function () {
            if (vm.pencadangan.pR_StartDate != vm.pencadangan.pR_MaturityDate) {
                ...
                vm.totalBeban = vm.pencadangan.pO_NominalBarang + (10 / 100 * vm.pencadangan.pO_NominalBarang)
        ...
        vm.delete = function () {
            vm.saving = true;
            PencadanganService.deleteData(vm.pencadangan)
				.then(function () {
				    //Close the modal
				    $scope.$parent.$close();
				})
