(function () {
    window.app.factory('BreakAmortisasiRequestService', BreakAmortisasiRequestService);

    BreakAmortisasiRequestService.$inject = ['$http', 'DataService', 'alerts'];
    function BreakAmortisasiRequestService($http, DataService, alerts) {

        var controllerPath = 'BreakAmortisasiRequest/'
        var list = [];
        ...
        function getDeliveryOrder(params) {
            return DataService.post(controllerPath + 'getAmortisasiRequest', params)
				.then(function (response) {
				    return response;
				});
        }
