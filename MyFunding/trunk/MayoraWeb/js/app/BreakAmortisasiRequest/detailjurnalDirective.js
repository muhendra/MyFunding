(function () {
    ...
    window.app.directive('detailjurnalBreakamortisasirequest', detailjurnalBreakamortisasirequest);
    function detailjurnalBreakamortisasirequest() {
        return {
            scope: {
                data: '='
            },
            templateUrl: MyApplication.rootPath + '/BreakAmortisasiRequest/template/detailjurnal.tmpl.cshtml',
            controller: controller,
            controllerAs: 'vm'
        }
    }

    controller.$inject = ['$scope', '$modal', 'BreakAmortisasiRequestService', 'branchService'];
    function controller($scope, $modal, BreakAmortisasiRequestService, branchService) {
        var vm = this;
        vm.saving = false;
        ...
        vm.getBranceName = function () {
            if (vm.detailjurnal.branchId != null && vm.detailjurnal.branchId != '') {
                BreakAmortisasiRequestService.descbranch(vm.detailjurnal.branchId).then(function (val) {
        ...
        vm.getCatName = function () {
            if (vm.detailjurnal.intAccCategoryId != null && vm.detailjurnal.intAccCategoryId != '') {
                BreakAmortisasiRequestService.descintaccount(vm.detailjurnal.intAccCategoryId).then(function (val) {
        ...
        vm.getBranceName2 = function () {
            if (vm.detailjurnal2.branchId != null && vm.detailjurnal2.branchId != '') {
                BreakAmortisasiRequestService.descbranch(vm.detailjurnal2.branchId).then(function (val) {
        ...
        vm.getCatName2 = function () {
            if (vm.detailjurnal2.intAccCategoryId != null && vm.detailjurnal2.intAccCategoryId != '') {
                BreakAmortisasiRequestService.descintaccount(vm.detailjurnal2.intAccCategoryId).then(function (val) {
        ...
        vm.save = function () {
            if (vm.detailjurnal.amount != vm.detailjurnal2.amount) {
                vm.message.error("Amount debit dan Kredit Harus sama!");
                return false;
            }
            ...
            if (vm.detailjurnal.amount == 0 || vm.detailjurnal2.amount == 0) {
                vm.message.error("Amount harus di isi!");
                return false;
            }
            vm.saving = true;
            vm.model = [];
