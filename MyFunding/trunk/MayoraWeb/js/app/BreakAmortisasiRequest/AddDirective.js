(function () {
    ...
    window.app.directive('addBreakamortisasirequest', addBreakamortisasirequest);

    function addBreakamortisasirequest() {
        return {
            templateUrl: MyApplication.rootPath + '/breakamortisasirequest/template/add.tmpl.cshtml',
    ...
    function controller($scope, $modal, Upload, $timeout, BreakAmortisasiRequestService, employeeService, departmentService) {
        ...
        vm.onSelectJurnalNameCombobox = function () {
            
        }

        vm.breakamortisasirequest.requestDate = new Date();
        ...
        function () {
            ...
            if (vm.breakamortisasirequest.pR_JenisTabungan = 6502 /*si pucuk*/) {
                vm.selectedJurnalName = { id: 1, name: 'Break Amortisasi Tabungan Si Pucuk', transaksi: 'BreakAmortisasiSiPucuk' }
                vm.breakamortisasirequest.jurnalName = vm.selectedJurnalName.name;
                vm.breakamortisasirequest.transaksi = vm.selectedJurnalName.transaksi;
            }
            else if (vm.breakamortisasirequest.pR_JenisTabungan = 6501 /*berhadiah*/) {
                vm.selectedJurnalName = { id: 2, name: 'Break Amortisasi Tabungan Berhadiah', transaksi: 'BreakAmortisasiBerhadiah' }
                vm.breakamortisasirequest.jurnalName = vm.selectedJurnalName.name;
                vm.breakamortisasirequest.transaksi = vm.selectedJurnalName.transaksi;
            }
            ...
            vm.kartuAmortisasi = vm.selectedDeliveryOrder.listKartuAmortisasi;
            vm.totalBeban = vm.selectedDeliveryOrder.pO_TotalPurchase / 0.8; //+ (20 / 100 * vm.selectedDeliveryOrder.pO_NominalBarang);

            vm.breakamortisasirequest.jurnalDetail = [];
            if (vm.selectedJurnalName.transaksi != null) {
                BreakAmortisasiRequestService.processJurnalOtomatis({ poNo: vm.selectedDeliveryOrder.pO_POId, narrative: vm.breakamortisasirequest.requestId, transaksi: vm.selectedJurnalName.transaksi }).then(function (data) {

                    for (var i = 0; i < data.data.length; i++) {
                        //data.data[i].amount = vm.bebanPertamax;
                        vm.breakamortisasirequest.jurnalDetail.push(data.data[i]);
                    }
                    vm.refreshData();
                });
            }
        ...
        function save() {
            ...
            vm.saving = true;
            BreakAmortisasiRequestService.add(vm.breakamortisasirequest)
                    .then(function (data) {
