(function () {
    ...
    window.app.directive('deleteBreakamortisasirequest', deleteBreakamortisasirequest);
    function deleteBreakamortisasirequest() {
        return {
            scope: {
                data: '='
            },
            templateUrl: MyApplication.rootPath + '/breakamortisasirequest/template/delete.tmpl.cshtml',
    ...
    function controller($scope, $modal, BreakAmortisasiRequestService) {
        ...
        vm.delete = function () {
            vm.saving = true;
            BreakAmortisasiRequestService.deleteData(vm.breakamortisasirequest)
				.then(function () {
				    //Close the modal
				    $scope.$parent.$close();
				})
