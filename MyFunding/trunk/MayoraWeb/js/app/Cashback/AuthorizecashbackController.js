(function() {
	...
	window.app.controller('AuthorizecashbackController', AuthorizecashbackController);

	AuthorizecashbackController.$inject = ['$scope', '$modal', 'PurchaseorderService', 'CashbackService', 'branchService', 'alerts'];
	function AuthorizecashbackController($scope, $modal, PurchaseorderService, CashbackService, branchService, alerts) {
	    var vm = this;
	    vm.search = search;
		vm.add = add;
		vm.detail = detail;
		...
		PurchaseorderService.getBranchUserLogin().then(function (value) {
		    ...
		    vm.authorizeCashbackGetPage = CashbackService.authorizegetPage;
		})
		...
		function detail(data) {
		    $modal.open({
		        ...
		        template: '<history-cashback data="data" />',
		        scope: angular.extend($scope.$new(true), { data: data })
		    });
		}
	}
