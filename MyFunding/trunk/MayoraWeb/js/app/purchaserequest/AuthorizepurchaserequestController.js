(function() {
	...
	window.app.controller('AuthorizepurchaserequestController', AuthorizepurchaserequestController);

	AuthorizepurchaserequestController.$inject = ['$scope', '$modal', 'PurchaserequestService', 'branchService', 'alerts'];
	function AuthorizepurchaserequestController($scope, $modal, PurchaserequestService, branchService, alerts) {
	    var vm = this;
	    vm.search = search;
		vm.add = add;
		vm.detail = detail;
		...
		//vm.authorizePurchaseRequestGetAll = PurchaserequestService.AuthorizeGetAll;
		...
		PurchaserequestService.getBranchUserLogin().then(function (value) {
		    vm.purchaserequest.cabang = value.branchCodeT24;
		    if (value.isKpno == 'no') {
		...
		function search() {
		    ...
		    //PurchaserequestService.authorizegetPage(vm.searchParams).then(function (value) {
		    //    vm.authorizepurchaserequestGetPage = value.data;
		        vm.refreshData();
		    //})
		...
		function reject(data) {
		    ...
		    //    PurchaserequestService.reject(data)
            //            .then(function (data) {
		    ...
		    var modalInstance = $modal.open({
		        ...
		        template: '<rejectreason-purchaserequest data="data" />',
		...
		function deleted(data) {
		    if (confirm("Are you sure for delete?")) {
		        ...
		        PurchaserequestService.deleted(data)
                        .then(function (data) {
		...
		function approve(data) 
		{
		    if (confirm("Are you sure for authorize?")) {
		            ...
		            vm.saving = true;
		            PurchaserequestService.approve(data)
                            .then(function (data) {
		...
		function edit(data) {
		    var modalInstance = $modal.open({
		        ...
		        template: '<editreject-purchaserequest data="data" />',
		        scope: angular.extend($scope.$new(true), { data: data })
		    });
