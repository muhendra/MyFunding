...
namespace GetDataPDF
{
    ...
    {
        public static Document CreatePDF(string accountno, string periode, string pathserver)
        {
            ...
            try
            {
                ...
                var a = dbContext.HeaderTextFileToPDFSet(accountno, periode).ToList(); 
                //.ExecDataTableProc("[Get_Header_TextFile_ToPDF]", "@AccountNo", accountno).ToList<GetHeaderTextFileToPDF>();
                foreach (var item in a)
                {
                    ...
                    //Image
                    string Logo = pathserver+"Images\\logo_header.png";
                    MigraDoc.DocumentObjectModel.Shapes.Image image = section.Headers.Primary.AddImage(Logo);
                    image.Height = "1.5cm";
                    image.Width = "6cm";
                    ...
                    var listbody = dbContext.BodyTextFileToPdfSet(item.ID).ToList(); 
